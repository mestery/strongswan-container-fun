...
if [ "$1" == 'exabgp' ]; then
  ...
  if [ -f /usr/etc/exabgp/exabgp.conf.client1 ] ; then
    cp /usr/etc/exabgp/exabgp.conf.client1 /usr/etc/exabgp/exabgp.conf
  fi
  if [ -f /usr/etc/exabgp/exabgp.conf.client2 ] ; then
    cp /usr/etc/exabgp/exabgp.conf.client2 /usr/etc/exabgp/exabgp.conf
  fi

  # Modify the default route
  route delete default
  route add default gw 10.199.199.40
  ...
  env exabgp.daemon.user=root /usr/bin/exabgp /usr/etc/exabgp/exabgp.conf
else 
  exec "$@"
fi
